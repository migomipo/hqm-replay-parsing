...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    let bytes = reader.read_u32_aligned() as usize;
...
fn read_objects(reader: & mut HQMMessageReader, history: & mut HashMap<u32, Vec<HQMObjectPacket>>) -> (Vec<HQMGameObject>, u32) {
    ...
    let objects = packets.iter().map(|x| match x {
